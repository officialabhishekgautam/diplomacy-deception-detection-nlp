...
class DiplomacyDataset:
    ...
    def get_class_weights(self):
        ...
        total = len(labels)
        weights = {0: total / (2 * class_counts[0]), 1: total / (2 * class_counts[1])}
        return weights
...
class HierarchicalDeceptionModel(nn.Module):
    def __init__(self, bert_model_name, metadata_dim, hidden_size=128, dropout=0.5):
        ...
        for param in list(self.bert.encoder.layer[-2:].parameters()):
            param.requires_grad = True
            
        # Message-level BiLSTM
        ...
        # Feature integration - REPLACE BatchNorm with LayerNorm
        self.feature_integration = nn.Sequential(
            nn.Linear(hidden_size * 2 + metadata_dim, hidden_size * 2),
            nn.LayerNorm(hidden_size * 2),  # Using LayerNorm instead of BatchNorm
            nn.ReLU(),
            nn.Dropout(dropout)
        )
        ...
        for i in range(batch_size):
            ...
            if valid_len > 0:
                ...
                for j in range(valid_len):
                    ...
                    combined_feature = torch.cat([context_output[j], dialogue_metadata[j]], dim=0)
...
def train_epoch(model, train_loader, optimizer, criterion, device):
    ...
    for batch in tqdm(train_loader, desc="Training"):
        ...
        # Forward pass
        outputs = model(input_ids, attention_mask, metadata, dialogue_lens)
        
        # Flatten labels for loss calculation
        flat_labels = []
        for i, length in enumerate(dialogue_lens):
            flat_labels.extend(labels[i, :length].tolist())
        flat_labels = torch.tensor(flat_labels).to(device)
...
def validate(model, val_loader, criterion, device):
    ...
    class_precision, class_recall, class_f1, _ = precision_recall_fscore_support(true_labels, predictions)
    ...
    metrics = {
        ...
        'recall': recall,
        'f1': f1,
        'truthful_precision': class_precision[0],
        'truthful_recall': class_recall[0],
        'truthful_f1': class_f1[0],
...
def analyze_errors(predictions, true_labels, val_df):
    ...
    error_examples['predicted'] = predictions[error_indices]
    error_examples['true'] = true_labels[error_indices]
...
def main():
    ...
    print("\nClass distribution:")
    print(f"Train: {sum(train_labels)} deceptive, {len(train_labels) - sum(train_labels)} truthful")
    print(f"Validation: {sum(val_labels)} deceptive, {len(val_labels) - sum(val_labels)} truthful")
    print(f"Test: {sum(test_labels)} deceptive, {len(test_labels) - sum(test_labels)} truthful")
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Truthful - Precision: {val_metrics['truthful_precision']:.4f}, Recall: {val_metrics['truthful_recall']:.4f}, F1: {val_metrics['truthful_f1']:.4f}")
        print(f"Deceptive - Precision: {val_metrics['deceptive_precision']:.4f}, Recall: {val_metrics['deceptive_recall']:.4f}, F1: {val_metrics['deceptive_f1']:.4f}")
    ...
    print(f"Test Truthful - F1: {test_metrics['truthful_f1']:.4f}")
    print(f"Test Deceptive - F1: {test_metrics['deceptive_f1']:.4f}")
